...
This trainer        return return_imgs
...
class StructuralLoss(nn.Module):
    ...
    def edge_consistency_loss(self, mask, fluorescent):
        """Ensure fluorescent edges align with mask edges"""
        mask_edges = self.compute_edges(mask)
        fluor_edges = self.compute_edges(fluorescent)
        
        # L1 loss between edge maps
        return F.l1_loss(fluor_edges, mask_edges)
    
    def spatial_correlation_loss(self, mask, fluorescent):
        """Ensure spatial correlation between mask and fluorescent intensity"""
        # Normalize to [0, 1] for correlation calculation
        mask_norm = (mask + 1.0) / 2.0
        fluor_norm = (fluorescent + 1.0) / 2.0
        
        # Compute spatial correlation
        mask_flat = mask_norm.view(mask_norm.size(0), -1)
        fluor_flat = fluor_norm.view(fluor_norm.size(0), -1)
...
class CycleGANTrainer:lements the complete CycleGAN training loop for translating
...
class CycleGANTrainer:
    ...
    def __init__(self, mask_dir, fluorescent_dir, image_size=256, 
        ...
        Args:
            ...
            m2f_emphasis: Emphasis factor for M→F direction (>1.0 gives more weight to M→F)
        """
        ...
        self.lr = lr
        self.beta1 = beta1
        self.m2f_emphasis = m2f_emphasis
    ...
    def train_step(self, real_masks, real_fluorescent):
        ...
        with autocast(enabled=self.scaler_G is not None):
            ...
            # Apply emphasis to M→F direction for better mask-to-fluorescent generation
            g_m2f_weighted = g_m2f_loss['total_loss'] * self.m2f_emphasis
            g_f2m_weighted = g_f2m_loss['total_loss']
            
            # Total generator loss with M→F emphasis
            g_total_loss = g_m2f_weighted + g_f2m_weighted
    ...
    def train(self, num_epochs, batch_size, save_interval=10, output_dir='./cyclegan_outputs', num_workers=4):
        ...
        print(f"  M→F emphasis factor: {self.m2f_emphasis:.1f}x (for better mask→fluorescent generation)")
...
def main():
    ...
    parser.add_argument('--cycle_loss_weight', type=float, default=10.0,
                      help='Weight for cycle consistency loss (try 20-50 for better mask generation)')
    parser.add_argument('--identity_loss_weight', type=float, default=0.5,
                      help='Weight for identity loss')
    parser.add_argument('--f2m_cycle_weight', type=float, default=None,
                      help='Separate cycle weight for F→M direction (if None, uses cycle_loss_weight)')
    parser.add_argument('--mask_segmentation_loss', action='store_true',
                      help='Add additional segmentation loss for F→M direction')
    parser.add_argument('--m2f_emphasis', type=float, default=2.0,
                      help='Emphasis factor for M→F direction (>1.0 gives more weight to mask→fluorescent, try 2.0-5.0)')
    ...
    trainer = CycleGANTrainer(
        ...
        m2f_emphasis=args.m2f_emphasis
    )
